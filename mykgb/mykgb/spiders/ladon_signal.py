...
class LadonSignalSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        print(df_buy)
        print(df_sell)
        sm(u"加权做多信号 ", self.send_signal_buy(df_buy), self.receiver, self.msg_cc)
        sm(u"加权做空信号 ", self.send_signal_sell(df_sell), self.receiver, self.msg_cc)

    def send_signal_buy(self, df):
        signal = ''
        for index, row in df.iterrows():
            signal += u'<h3 STYLE="color:red;">做多 ' + row['code'] + '</h3>'
    ...
    def send_signal_sell(self, df):
        signal = ''
        for index, row in df.iterrows():
            signal += u'<h3 STYLE="color:green;">做空 ' + row['code'] + '</h3>'
            signal += u'<p>强度: ' + str(int(row['position'])) + '</p>'
        return signal
